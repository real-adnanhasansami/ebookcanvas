...
let statusEl = document.getElementById("flipbook-status");
...
async function openBook(previewCap) {
  // Audiobook: only available if book has audio AND user is active Premium
...
async function persistNotes(statusDisplay, silent) {
  if (statusDisplay && !silent) statusDisplay.textContent = "Saving…";
  try {
  ...
  } catch (e) { if (statusDisplay) statusDisplay.textContent = "Couldn't save."; }
}
