...
let pendingFile = null, compressedBytes = null, pendingCoverBlob = null;
...
document.getElementById("pdf-input").addEventListener("change", async (e) => {
  ...
  generateCoverThumbnail(new Uint8Array(bytes)).then((blob) => { pendingCoverBlob = blob; });
...
document.getElementById("publish-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    await addDoc(collection(db, "books"), {
      title, author, coverURL: finalCoverURL, fileURL,
      isPremiumBook, hasAudio, published: true,
      readCount: 0,
    ...
    pendingFile = null; compressedBytes = null; pendingCoverBlob = null;
    loadBooks();
  } catch (err) {
