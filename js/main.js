...
export async function fetchPublishedBooks() {
  ...
  try {
    ...
    console.error("fetchPublishedBooks error:", e);
    allBooksCache = [];
  }
...
async function renderHomeLibraryPreview(user, profile) {
  ...
  // Guests see the library but clicking redirects to login
  if (!user) {
  ...
  if (!user) { if (continueSection) continueSection.style.display = "none"; return; }
  if (continueShelf && continueSection) {
    ...
    if (inProgress.length === 0) {
      ...
      // Show only last 3 books read (sort by page number desc as proxy for recency)
      const recent = inProgress
        .sort((a, b) => (progress[b.id] || 0) - (progress[a.id] || 0))
        .slice(0, 3);
      continueShelf.innerHTML = recent.map(bookCardHTML).join("");
    }
  }
}
