...
document.getElementById("publish-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    const hint = document.getElementById("cloudinary-hint");
...
async function loadDashboardStats() {
  try {
    ...
    let premiumCount = 0;
    usersSnap.forEach((d) => { if (d.data().isPremium) premiumCount++; });
    const pendingCount = subsSnap.docs.filter((d) => d.data().status === "pending").length;
    document.getElementById("stat-users").textContent = usersSnap.size;
    document.getElementById("stat-premium").textContent = premiumCount;
    document.getElementById("stat-sub-requests").textContent = pendingCount;
